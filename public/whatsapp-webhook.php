...
// ── Seguridad ──────────────────────────────────────────────────
// 1. Solo GET (verificación) y POST (mensajes) permitidos
if (!in_array($_SERVER['REQUEST_METHOD'], ['GET', 'POST'])) {
    http_response_code(405); exit;
}

// 2. Los POST deben venir con firma válida de Meta
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $rawBody   = file_get_contents('php://input');
    $signature = $_SERVER['HTTP_X_HUB_SIGNATURE_256'] ?? '';
    $appSecret = '__APP_SECRET__'; // Clave secreta de la app Meta
    ...
    // Pasar el body ya leído al resto del script
    $GLOBALS['_RAW_BODY'] = $rawBody;
}

// 3. Solo acepta User-Agent de Meta
$ua = $_SERVER['HTTP_USER_AGENT'] ?? '';
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !str_contains($ua, 'facebookplatform') && !str_contains($ua, 'Meta')) {
    // Permitir en desarrollo — comentar en producción si quieres más strictness
...
if ($type === 'text') {
    ...
    // Tipo no soportado (imagen, video, etc.)
...
// ── Construir system prompt (espejo de ntdSys() en nova_TD.html) ──
...
$raw = callClaude($systemPrompt, $history);
...
// ── Construir system prompt (PHP mirror de ntdSys()) ──────────
...
function callClaude(string $system, array $messages): string {
    $payload = json_encode([
...
// ── Consultar radicado PQRSFD en Supabase ─────────────────────
...
function parseAction(string $raw): array {
    $tags = [
        'FORMULARIO', 'ESCALAR', 'ENCUESTA', 'MENU',
        'MEDICAMENTOS', 'REQUISITOS', 'CAMBIAR_EPS',
    ];

    // Tags con datos: [CONSULTAR:valor], [SEDES:municipio], [SEDES:municipio:EPS]
