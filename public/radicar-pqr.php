...
$eps_paciente   = trim($body['eps_paciente']   ?? '');
$ciudad_paciente_form = trim($body['ciudad_paciente'] ?? '') ?: $sede_ciudad;
...
if (in_array($origen, ['nova_web', 'nova_directo', 'nova_td'])) {
    $origen_label = ($origen === 'nova_directo') ? '🤖 NOVA DIRECTO' : '🤖 NOVA (ENLACE)';
...
try {
    $sb_payload = [
        ...
        'body_content'      => $descripcion ?: $transcripcion ?: null,
        ...
        'ciudad_paciente'   => $ciudad_paciente_form ?: null,
...
<table width='580' cellpadding='0' cellspacing='0' style='max-width:580px;width:100%;background:#ffffff'>
  ...
  <tr><td style='background:#0c2d5e;padding:18px 44px'>
    <table width='100%' cellpadding='0' cellspacing='0'>
      <tr>
        <td style='font-size:10px;color:#4a6a90;line-height:1.6'>Tododrogas<br>Experiencia del Servicio &middot; Nova TD v4</td>
    ...
    if ($correo_id && $cuerpo_html) {
        try {
            ...
            curl_setopt_array($ch_patch, [
                ...
                CURLOPT_POSTFIELDS     => json_encode(['body_content' => $cuerpo_html, 'body_type' => 'html']),
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_TIMEOUT        => 10,
                CURLOPT_HTTPHEADER     => [
                    "apikey: $SB_KEY",
...
<table width='580' cellpadding='0' cellspacing='0' style='max-width:580px;width:100%;background:#ffffff'>
  ...
  <tr><td style='background:#0c2d5e;padding:18px 44px'>
    <table width='100%' cellpadding='0' cellspacing='0'>
      <tr>
        <td style='font-size:10px;color:#4a6a90;line-height:1.6'>Tododrogas<br>Experiencia del Servicio &middot; Nova TD</td>
...
12. PROHIBIDO usar la palabra drogueria — reemplazala por punto de atencion o Tododrogas.
